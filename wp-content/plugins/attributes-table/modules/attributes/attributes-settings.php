...
function attributes_menu() {
	?>
	<a id="attributes-settings" class="nav-tab"  href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=attributes-settings&submenu=attributes' ) ?>"><?php _e( 'Attributes', 'attributes-table' ); ?></a>
	<?php
}
...
function attributes_settings_content() {
	...
	<div class="attributes-product-settings settings-wrapper" style="clear:both;">
		<div class="settings-submenu">
			<h3>
				<a id="attributes-settings" class="element current" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=attributes-settings&submenu=attributes' ) ?>"><?php _e( 'Attributes Settings', 'attributes-table' ); ?></a>
		...
		<?php if ( $submenu == 'attributes' ) { ?>
			<div class="setting-content submenu">
				...
				<form method="post" action="options.php">
					...
					<table>
						<tr>
							<td colspan="2"><?php _e( 'Number of attributes', 'attributes-table' ); ?> <input size="30" type="number" step="1" min="0" name="product_attributes_number" id="admin-number-field" value="<?php echo $attributes_count; ?>" /><input type="submit" class="button" value="<?php _e( 'Update', 'attributes-table' ); ?>" />
							</td>
						</tr>
					</table>
					<?php
					if ( $attributes_count > 0 ) {
						...
						<div class="al-box info">
							<p><?php _e( "If you fill out the fields below, system will automatically pre-fill the fields on when adding new item so you doesn't have to fill them every time again.</p><p>When every item in your catalogue is different you can leave all or a part of these field empty.", 'attributes-table' ); ?></p><p><?php _e( 'You can change these default values for every item.', 'attributes-table' ); ?></p>
						</div>
						<div class="settings-table-container" style="overflow-x: scroll;">
							<table  class="wp-list-table widefat product-settings-table dragable">
								<thead>
									<tr>
										<th class="title"></th><th class="title"><b><?php _e( 'Attribute name', 'attributes-table' ); ?></b></th>
										<th></th>
										<th class="title"><b><?php _e( 'Attribute value', 'attributes-table' ); ?></b></th>
					...
					<p class="submit">
						<input type="submit" class="button-primary" value="<?php _e( 'Save changes', 'attributes-table' ); ?>" />
					</p>
				</form>
			</div>
...
function ic_listing_attributes_settings( $listing_settings, $listing_name ) {
	?>
	<input title="<?php _e( 'Use this only with short attributes labels and values e.g. Color: Red', 'attributes-table' ) ?>" type="checkbox" name="<?php echo $listing_name ?>_settings[attributes]" value="1"<?php checked( 1, isset( $listing_settings[ 'attributes' ] ) ? $listing_settings[ 'attributes' ] : ''  ); ?>> <?php _e( 'Show Attributes', 'attributes-table' ); ?><br><?php
}
...
function ic_available_weight_units() {
	$units = array( 'disable' => __( 'Disable Weight', 'attributes-table' ), 'kg' => 'kg', 'g' => 'g', 'lbs' => 'lbs', 'oz' => 'oz' );
	return $units;
}

function ic_available_size_units() {
	$units = array( 'disable' => __( 'Disable Size', 'attributes-table' ), 'm' => 'm', 'cm' => 'cm', 'mm' => 'mm', 'in' => 'in', 'yd' => 'yd' );
	return $units;
}
...
function ic_attributes_standard_labels( $single_names ) {
	$single_names[ 'product_size' ]		 = __( 'Size', 'attributes-table' ) . ':';
	$single_names[ 'product_weight' ]	 = __( 'Weight', 'attributes-table' ) . ':';
	return $single_names;
}
