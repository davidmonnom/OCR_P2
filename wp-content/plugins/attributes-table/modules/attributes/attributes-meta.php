...
function al_product_attributes( $post, $args = null ) {
	...
	implecode_info( __( 'Only attributes with values set will be shown on product page.', 'attributes-table' ) . ' ' . sprintf( __( 'See <a target="_blank" href="%s">docs</a>.', 'attributes-table' ), 'https://implecode.com/docs/ecommerce-product-catalog/product-attributes/?cam=catalog-add-page-box&key=product-attributes' ) );
...
function ic_size_field_names() {
	$field_names = array( '_size_length' => _x( 'L', 'Length', 'attributes-table' ), '_size_width' => _x( 'W', 'Width', 'attributes-table' ), '_size_height' => _x( 'H', 'Height', 'attributes-table' ) );
	...
	if ( !is_ic_admin() && !$force_value ) {
		...
		foreach ( $field_names as $name => $label ) {
			...
			if ( $name === '_size_length' && !empty( $size[ 'length' ] ) ) {
				...
				$this_desc		 .= _x( 'W', 'Width', 'attributes-table' );
				$this_text_size	 .= $size[ 'width' ] . $unit;
			} else if ( $name === '_size_height' && !empty( $size[ 'height' ] ) ) {
				$this_desc		 .= _x( 'H', 'Height', 'attributes-table' );
...
function ic_size_metabox( $product_details, $product_id, $field_name = null, $size = null ) {
	if ( is_ic_attributes_size_enabled() ) {
		...
		if ( !empty( $field_names ) ) {
			...
			$fields	 = '<input placeholder="' . _x( 'L', 'Length', 'attributes-table' ) . '" style="max-width: 25%;" type="text" name="' . $field_name[ 0 ] . '" value="' . $size[ 'length' ] . '" class="widefat" />';
			$fields	 .= 'x <input placeholder="' . _x( 'W', 'Width', 'attributes-table' ) . '" style="max-width: 25%;" type="text" name="' . $field_name[ 1 ] . '" value="' . $size[ 'width' ] . '" class="widefat" />';
			$fields	 .= 'x <input placeholder="' . _x( 'H', 'Height', 'attributes-table' ) . '" style="max-width: 25%;" type="text" name="' . $field_name[ 2 ] . '" value="' . $size[ 'height' ] . '" class="widefat" />';
		}
...
function ic_weight_metabox( $product_details, $product_id, $field_name = '_weight', $weight = null ) {
	if ( is_ic_attributes_weight_enabled() ) {
		...
		$label	 = __( 'Weight', 'attributes-table' );
		$unit	 = ic_attributes_get_weight_unit();
		if ( !empty( $unit ) ) {
			$label .= ' (' . $unit . ')';
		}
