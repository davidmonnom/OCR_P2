...
//add_filter( 'ic_catalog_tax_nav_menu', array( $this, 'fake_tax_first_post' ), 99 );
	...
	function settings_html( $settings ) {
		...
		implecode_settings_dropdown( __( 'Template', 'post-type-x' ), 'archive_multiple_settings[shortcode_mode][template]', $settings[ 'template' ], array_merge( array( '0' => __( 'Default Template', 'post-type-x' ) ), $this->available_templates() ) );
	}
...
//Customizr
	...
	function shortcode_init() {
		...
		remove_filter( 'the_content', array( $this, 'auto_add_shortcode' ), 99999999 );
...
//add_action( 'the_content', array( $this, 'end_query' ) );
	...
	function default_message() {
		...
		if ( !empty( $page_url ) ) {
			...
			$message = '<p style="margin-bottom: 5px;font-weight: normal;">' . __( 'Currently no page is set to show main product listing. Would you like to show product listing here?', 'post-type-x' ) . '</p>';
		}
		if ( !empty( $message ) && !empty( $page_id ) ) {
			...
			$message .= '<p style="font-size: 0.8em">* ' . sprintf( __( 'Use the %s or %s shortcode if you just want to display products or categories.', 'post-type-x' ), '[[show_products]]', '[[show_categories]]' ) . '</p>';
