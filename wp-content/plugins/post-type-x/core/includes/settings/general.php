					...
					if ( !is_advanced_mode_forced() || ic_is_woo_template_available() || is_ic_shortcode_integration() ) {
						...
						<div class="ic-important-settings">
							...
							if ( ic_has_listing_shortcode() ) {
								implecode_info( sprintf( __( 'You are currently using %s on your product listing to integrate the catalog with the theme.', 'post-type-x' ), ic_catalog_shortcode_name() ) . '</p>' . '<p>' . __( 'If you have any problems with catalog layout you can remove the shortcode and use the theme integration wizard to fix it.', 'post-type-x' ) . '</p>' );
...
//echo sample_product_button( 'p', __( 'Remove the Shortcode and Start Visual Wizard', 'post-type-x' ), 'button' );
								...
								'<p>' . sprintf( __( 'Place %s in the text editor where you would like the product list to be displayed on your website.', 'post-type-x' ), ic_catalog_shortcode_name() ) . '</p>' .
					...
					<div class="advanced_mode_settings_inline">
						...
						<table>
							...
							implecode_settings_radio( __( 'Category Page shows', 'post-type-x' ), 'archive_multiple_settings[category_top_cats]', $archive_multiple_settings[ 'category_top_cats' ], array( 'off' => $uc_item_name, 'on' => $uc_item_name . ' & ' . __( 'Subcategories', 'post-type-x' ), 'only_subcategories' => __( 'Subcategories', 'post-type-x' ) ), 1, __( 'The main listing can show only products, top level categories and products or only the categories. With the subcategories option selected the products will show only if they are directly assigned to the category. If you want to display the products only on the bottom category level please assign the products only to it (not to all categories in the tree).', 'post-type-x' ) );
...
function ic_shortcode_mode_settings() {
	...
	if ( is_ic_shortcode_integration() ) {
		...
		implecode_settings_checkbox( __( 'Show main catalog page content everywhere', 'post-type-x' ), 'archive_multiple_settings[shortcode_mode][show_everywhere]', $settings[ 'show_everywhere' ] );
		implecode_settings_checkbox( __( 'Force product name display', 'post-type-x' ), 'archive_multiple_settings[shortcode_mode][force_name]', $settings[ 'force_name' ] );
		if ( is_ic_breadcrumbs_enabled() ) {
			implecode_settings_checkbox( __( 'Move breadcrumbs top the top', 'post-type-x' ), 'archive_multiple_settings[shortcode_mode][move_breadcrumbs]', $settings[ 'move_breadcrumbs' ] );
		}
