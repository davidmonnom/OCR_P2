...
class product_price_filter extends WP_Widget {
	...
	function widget( $args, $instance ) {
		if ( get_integration_type() != 'simple' ) {
			if ( ic_if_show_filter_widget( $instance ) ) {
				...
				?>
				<div class="price-filter ic_ajax <?php design_schemes( 'box' ) ?> <?php echo $class ?>" data-ic_ajax="price-filter" data-ic_responsive_label="<?php _e( 'Price', 'price-field' ) ?>">
	...
	function form( $instance ) {
		if ( get_integration_type() != 'simple' ) {
			...
			?>
			<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'price-field' ); ?> <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></label></p>
			<p><input class="widefat" id="<?php echo $this->get_field_id( 'shortcode_support' ); ?>" name="<?php echo $this->get_field_name( 'shortcode_support' ); ?>" type="checkbox" value="1" <?php checked( 1, $instance[ 'shortcode_support' ] ) ?> /> <label for="<?php echo $this->get_field_id( 'shortcode_support' ); ?>"><?php _e( 'Enable also for shortcodes', 'price-field' ); ?></label></p><?php
		} else {
