...
function ic_price_metabox( $product_details, $product_id ) {
	if ( is_ic_price_enabled() ) {
		...
		$product_details .= apply_filters( 'admin_price_table', '<table><tr><td class="label-column">' . __( 'Price', 'price-field' ) . ':</td><td class="price-column"><input type="text" title="' . sprintf( __( 'Example price format: %s or %s', 'price-field' ), price_format( '1587.89', 1, 0 ), '1587' . $set[ 'dec_sep' ] . '89' ) . ' (' . __( 'you can change it in product settings', 'price-field' ) . ')" pattern="^(([1-9](\\d*|\\d{0,2}(' . $set[ 'th_sep' ] . '\\d{3})*))|0)(' . $set[ 'dec_sep' ] . '\\d{1,2})?$" name="_price" value="' . $formatted_price . '" class="widefat" /></td><td>' . product_currency() . '</td></tr></table>', $product_id );
...
function ic_price_quickedit( $column_name ) {
	if ( $column_name == 'price' ) {
		?><span class="title"><?php _e( 'Price', 'price-field' ) ?></span><input type="number" min="0" step="0.01" name="_price" value="" class="widefat" /><?php
